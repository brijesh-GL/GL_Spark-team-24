...
function OrderCard({ name, price, img, orderby }) {
  return (
    <div className="orderCard__container">
      ...
      <div className="orderCard__right">
        <div>
          <h1 style={{fontSize:'20px', color:"#855E42",marginLeft:"80px"}}>{name.length < 30 ? name : `${name.slice(0, 30)} ...`}</h1>
          <h2 style={{fontSize:'20px', color:"black"}}>Seller: Atharva </h2>
        </div>
        <div>
          <h1 style={{fontSize:'20px', color:"#855E42",fontWeight:"bold"}} className="mobile-price">{`₹${price}`}</h1>
        </div>
        <div>
          <h1 style={{ display: "flex" ,fontSize:"20px" }}>
            <svg
              width="24"
              height="24"
